...
function JuAFEM.value(ip::QuadratischesDreieck, i::Int, ξ::Vec{2})
    ...
    ξ₂ = ξ[2]
    λ = 1. - ξ₁ - ξ₂
    i == 1 && return ξ₁ * (2ξ₁ - 1) # nur wenn i == 1 Rückgabe; if in Kurzfrom 
    i == 2 && return ξ₂ * (2ξ₂ - 1) # nur wenn i == 2 erfolgt diese Rückgabe, etc.
    i == 3 && return λ * (2λ - 1)
    i == 4 && return 4ξ₁ * ξ₂
    i == 5 && return 4ξ₂ * λ
    i == 6 && return 4ξ₁ * λ
...
function JuAFEM.reinit!(cv::CellVectorValues{dim}, x::AbstractVector{Vec{dim,T}}) where {dim,T}
    ...
    @inbounds for i in 1:length(cv.qr_weights) #Loop über alle Integrationspunkte
        ...
        cv.detJdV[i] = detJ * w
        Jinv = inv(fecv_J)
        for j in 1:n_func_basefuncs #Loop über alle Verschiebungs Ansatzfunktionen
...
begin
	...
	cellvalues_implementation = CellVectorValues(QuadratureRule{2, RefTetrahedron}(quad_order), QuadratischesDreieck())
