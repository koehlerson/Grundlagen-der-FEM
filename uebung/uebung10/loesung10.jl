...
function JuAFEM.gradient(ip::LinearesViereck, i::Int, ξ::Vec{2})
	ξ_x = ξ[1]
    ξ_y = ξ[2]
    i == 1 && return [ξ_y*0.25-0.25 , 0.25*ξ_x-0.25]
    i == 2 && return [0.25-ξ_y*0.25 , -0.25*(ξ_x+1)]
    i == 3 && return [0.25*(ξ_y + 1) , 0.25*(ξ_x + 1)]
    i == 4 && return [-0.25*(ξ_y + 1) , 0.25 - 0.25*ξ_x]
end
...
$(solution(md"Alle Gaußpunkte werden ausgewertet und in `CellVectorValues` gespeichert",blur=false))
...
# ╔═╡ afcb0876-60ce-11eb-2f9e-c3d442b6dd28
function triangle_quad_comparison(interpolation)
	if isa(interpolation, Lagrange{2,RefTetrahedron,1})
		meshcelltype = Triangle
	...
	corners = [Vec{2}((0.0,   0.0)),
				 ...
				 Vec{2}((0.0,  25.0))]
	nx,ny = 50, 50
	grid = generate_grid(meshcelltype, (nx, ny), corners);
	addfaceset!(grid, "clamped", x -> norm(x[1]) ≈ 0.0);
	addfaceset!(grid, "traction", x -> norm(x[1]) ≈ 50.0);
	dh = DofHandler(grid)
	push!(dh, :u, 2)
	close!(dh)
	ch = ConstraintHandler(dh)
	...
	close!(ch)
	update!(ch, 0.0)
	K, f = doassemble(elementvalues, randvalues, dh, E, ν, t=Vec{2}((100,0)))
	apply!(K, f, ch)
	u = K \ f

	σ = compute_stresses(elementvalues,dh,u,elasticity(E,ν))

	projector = L2Projector(integration, interpolation, dh.grid);

	σ_nodes = JuAFEM.project(σ, projector)

	plot_σ = [[σ_i[1], σ_i[1]] for σ_i in σ_nodes]
	plot_σ = vcat(plot_σ...)
	
	point1 = Vec(49.9,24.9)
	point2 = Vec(49.9,0.1)
	
	u_measured1 = measure_function(point1,u,dh,interpolation)
	u_measured2 = measure_function(point2,u,dh,interpolation)

	return dh,u, [u_measured1, u_measured2]
end

# ╔═╡ 2bbd9a5e-6229-11eb-1f1b-478dbb1a6211
dh_triangle, u_triangle, u_triangle_measured = triangle_quad_comparison(Lagrange{2,RefTetrahedron,1}())

# ╔═╡ a7f2ce5a-6229-11eb-342a-057b5afe278e
dh_quad, u_quad, u_quad_measured = triangle_quad_comparison(Lagrange{2,RefCube,1}())

# ╔═╡ 67a004f6-622b-11eb-0abd-ad29d76cdf1f
begin
	fig_u_triangle, ax_u_triangle, sp_u_triangle = JuAFEM.warp_by_vector(dh_triangle,u_triangle; colormap=:inferno, process=x->x[1])
	fig_u_triangle[1,2] = WGLMakie.Colorbar(fig_u_triangle; limits=(minimum(u_triangle),maximum(u_triangle)),colormap=:inferno, width=20)
	fig_u_triangle
end

# ╔═╡ 019c2cb4-622d-11eb-286a-b51667ab0e20
begin
	fig_u_quad, ax_u_quad, sp_u_quad = JuAFEM.warp_by_vector(dh_quad,u_quad; colormap=:inferno, process=x->x[1])
	fig_u_quad[1,2] = WGLMakie.Colorbar(fig_u_quad; limits=(minimum(u_quad),maximum(u_quad)),colormap=:inferno, width=20)
	fig_u_quad
end
